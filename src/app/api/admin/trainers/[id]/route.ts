...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (getDbMode() !== 'postgres') {
      const dbData = getDb();
      const trainers = (dbData?.trainers || []).map((t: any) =>
        String(t.id) === id ? {
          ...t,
          name: body.name ?? t.name,
          short_code: body.short_code ?? t.short_code,
          experience: body.experience ?? t.experience,
          description: body.description ?? t.description,
          specialization: body.specialization ?? t.specialization,
          isDirector: body.isDirector ?? t.isDirector,
          phone: body.phone ?? t.phone,
          image: body.image ?? t.image,
        } : t
      );
      saveDb({ ...dbData, trainers });
    ...
    try {
      ...
      console.warn('⚠️ PG недоступен, fallback на JSON (admin trainers PUT):', error);
      const dbData = getDb();
      const trainers = (dbData?.trainers || []).map((t: any) =>
        String(t.id) === id ? {
          ...t,
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (getDbMode() !== 'postgres') {
      const dbData = getDb();
      const trainers = (dbData?.trainers || []).filter((t: any) => String(t.id) !== id);
      saveDb({ ...dbData, trainers });
    ...
    try {
      ...
      console.warn('⚠️ PG недоступен, fallback на JSON (admin trainers DELETE):', error);
      const dbData = getDb();
      const trainers = (dbData?.trainers || []).filter((t: any) => String(t.id) !== id);
      saveDb({ ...dbData, trainers });
      return NextResponse.json({ success: true });
